...
TEXT_API_KEY = "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"
...
def pesquisar_na_internet(termo_busca):
    """Faz uma busca rápida no DuckDuckGo e extrai os trechos relevantes."""
    url = f"https://html.duckduckgo.com/html/?q={requests.utils.quote(termo_busca)}"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    try:
        response = requests.get(url, headers=headers, timeout=5)
        if response.status_code == 200:
            # Encontra os snippets de resultados usando regex simples sobre o HTML recebido
            snippets = re.findall(r'<a class="result__snippet"[^>]*>(.*?)</a>', response.text, re.DOTALL)
            resultados = []
            for snip in snippets[:4]:
                # Limpa tags HTML residuais
                texto_limpo = re.sub(r'<[^>]+>', '', snip).strip()
                resultados.append(texto_limpo)
            if resultados:
                return "\n".join(resultados)
    except Exception:
        pass
    return "Não foi possível obter dados adicionais recentes da internet para este termo."
...
if "current_chat" not in st.session_state:
    ...
    st.session_state.chats[primeiro_id] = [
        {
            "role": "system", 
            "content": f"Você é a Amoria, uma IA educacional didática altamente atualizada que fala Português. ATENÇÃO: O tempo avançou e HOJE É EXATAMENTE DIA {data_hoje} (Ano de 2026). Não use datas antigas de 2024 ou 2025. Aceite e use 2026 como o ano presente absoluto para todas as suas respostas. Se receber dados do sistema sobre a internet, use-os para complementar informações em tempo real."
        }
    ]
            ...
            for i, q in enumerate(st.session_state.questoes):
                ...
                if q['tipo'] == "multipla":
                    ...
                    if resp_usuario == correta:
                        st.success(f"✅ **Sua resposta:** {resp_usuario} — Você acertou!")
                        acertos += 1
                    else:
                        st.error(f"❌ **Sua resposta:** {resp_usuario} — O correto era **{correta}**.")
                
                elif q['tipo'] == "escrita":
                    if not resp_usuario.strip():
                        st.warning("⚠️ Você deixou esta questão em branco.")
                        st.error("Nota para esta questão: 0/1")
                    else:
                        with st.spinner(f"Amoria está analisando sua resposta da Questão {i+1}..."):
                            prompt_correcao = f"""
                            Você é um professor corrigindo uma questão dissertativa.
                            Pergunta: {q['pergunta']}
                            Resposta do aluno: {resp_usuario}
                            
                            Analise a resposta de forma breve e didática. 
                            Determine uma nota de 0.0 a 1.0 para esta resposta.
                            
                            Retorne APENAS um JSON no seguinte formato:
                            {{
                              "analise": "Texto curto explicando se a resposta está certa, incompleta ou errada.",
                              "nota_questao": 0.8
                            }}
                            """
                            payload_analise = {
                            ...
                            try:
                                res_corr = requests.post(TEXT_URL, headers=headers_text, json=payload_analise)
                                conteudo_corr = res_corr.json()['choices'][0]['message']['content'].replace('```json', '').replace('```', '').strip()
                                resultado_corr = json.loads(conteudo_corr)
                                
                                nota_escrita = float(resultado_corr.get("nota_questao", 0))
                                analise_ia = resultado_corr.get("analise", "Sem análise disponível.")
                                
                                acertos += nota_escrita
                                
                                if nota_escrita >= 0.7:
                                    st.success(f"📝 **Sua resposta:** {resp_usuario}\n\n**Análise da Amoria (Nota {nota_escrita:.1f}/1.0):** {analise_ia}")
                                elif nota_escrita >= 0.4:
                                    st.warning(f"📝 **Sua resposta:** {resp_usuario}\n\n**Análise da Amoria (Nota {nota_escrita:.1f}/1.0):** {analise_ia}")
                                else:
                                    st.error(f"📝 **Sua resposta:** {resp_usuario}\n\n**Análise da Amoria (Nota {nota_escrita:.1f}/1.0):** {analise_ia}")
                                    
                            except Exception as e:
                                st.error(f"⚠️ Não foi possível corrigir esta questão automaticamente: {e}")
                                st.info(f"Sua resposta enviada: {resp_usuario}")
                st.divider()
    ...
    if prompt := st.chat_input("Como posso ajudar nos seus estudos?"):
        ...
        with st.chat_message("assistant"):
            if gerar_imagem:
                ...
                # --- NOVO: CONSULTA EM TEMPO REAL NA INTERNET ---
                with st.spinner("Amoria está consultando fontes em tempo real na internet..."):
                    contexto_internet = pesquisar_na_internet(prompt)
                ...
                # Inserção do contexto de pesquisa em tempo real como comando de sistema dinâmico
                historico_texto_apenas.append({
                    "role": "system",
                    "content": f"INFORMAÇÕES RECENTES COLETADAS DA INTERNET EM TEMPO REAL:\n{contexto_internet}\nUse os dados acima se forem úteis para responder com exatidão a pergunta atual do usuário."
                })
