...
if "current_chat" not in st.session_state:
    ...
    # Captura a data atual do sistema (ex: 19/06/2026)
    data_hoje = datetime.date.today().strftime("%d/%m/%Y")
    
    st.session_state.chats[primeiro_id] = [
        {
            "role": "system", 
            "content": f"Você é uma IA educacional didática que fala Português. Seu nome é Amoria, e seu objetivo é explicar conteúdos de forma clara e objetiva. SAIBA QUE HOJE É DIA {data_hoje}. Sempre use esta data como referência para o tempo presente."
        }
    ]
...
with st.sidebar:
    ...
    if st.button("➕ Nova Conversa", use_container_width=True):
        ...
        data_hoje = datetime.date.today().strftime("%d/%m/%Y") # Pega a data atual
...
if st.session_state.modo_form:
    ...
    # TELA B: RESOLUÇÃO DO FORMULÁRIO (Questões já geradas)
        ...
        if st.button("Finalizar, Corrigir e Salvar"):
            ...
            # Criamos um container para mostrar o progresso da correção das escritas
            status_correcao = st.empty()
            ...
            for i, q in enumerate(st.session_state.questoes):
                ...
                if q['tipo'] == "multipla":
                    ...
                    # Nova lógica: Chamada à API para corrigir a questão dissertativa
            ...
            # Cálculo da nota final ajustado para somar as notas das escritas e múltiplas escolhas
