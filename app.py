...
def responder_conversa_rapida(mensagem):
    prompt_sistema = (
        "Você é a Professora Amoria, uma assistente virtual educacional amigável, prestativa e direta. "
        "IMPORTANTE: Você deve responder SEMPRE em português do Brasil (pt-BR)."
    )
    ...
    try:
        res = requests.post(TEXT_URL, headers=headers_text, json=payload, timeout=10)
        if res.status_code == 200:
            return res.json()['choices'][0]['message']['content']
    except Exception:
        pass
        
    return "Olá! Como posso ajudar você com os estudos hoje?"
    ...
    if prompt := st.chat_input("Pergunte algo..."):
        ...
        with st.chat_message("assistant"):
            if gerar_imagem:
                ...
                if e_conversa_curta:
                    resposta_rapida = responder_conversa_rapida(prompt)
